...
function Get-DockerNetwork {
  ...
  if ($Name.Length -eq 0) {
    ...
    foreach ($line in $net) {
      ...
      $network | Add-Member -MemberType NoteProperty -Name Id `
        -Value $result.Matches.Groups[1].Value
      $network | Add-Member -MemberType NoteProperty -Name Name `
        -Value $result.Matches.Groups[2].Value
      $network | Add-Member -MemberType NoteProperty -Name Driver `
        -Value $result.Matches.Groups[3].Value
      $network | Add-Member -MemberType NoteProperty -Name Scope `
        -Value $result.Matches.Groups[4].Value
